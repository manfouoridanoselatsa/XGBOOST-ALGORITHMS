	...
	public:
		...
		void find_varsplit() {
		    ...
		    while(!node_stack.empty()){
		    	...
		    	find_greedy_split(curr_depth,node_idx);
		    }
		    
		}
		
		void find_greedy_split(int &curr_depth,int node_idx){
			...
			selected.reserve((int)(this->subsample_cols * this->col_count));
			for(int i = 0; i < (int)(this->subsample_cols * this->col_count); i++) {
				selected.emplace_back(permutation[i]);
			}
			
			/*int permut_size =  (int)(this->col_count *  this->subsample_cols);
			
			std::vector<int> permutation(this->col_count);
			std::vector<int> selected(permut_size);
			...
			for (int i = permut_size - 1; i >= 1; i--) {
				int j = rand() % (this->col_count);
				selected[i] = permutation[j];
			}*/
			
			
			this->column_subsample = selected;
			...
			for(int i : tree[node_idx].idxs){
				gradient.emplace_back(this->gradient[i]);
				hessian.emplace_back(this->hessian[i]);
				//std::cout<<i<<" ";		
			}
			//std::cout<<std::endl;
			
			tree[node_idx].val = this->compute_gamma(gradient,hessian);
				...
				for (int r = 0; r < xsplit_size; r++){
					...
					for (int i = 0; i < xsplit_size; i++) {
						if (xsplit[i] <= xsplit[r]) {
							lhs[i] = true;
							lhs_sum++;
							lhs_indices.emplace_back(i);
							lhs_hessian_sum += hessian[i];
						} else  {
							rhs[i] = true;
							rhs_sum++;
							rhs_indices.emplace_back(i);
							rhs_hessian_sum += hessian[i];
						}
					}
			...
			tree.emplace_back(Node(lhsF,curr_depth+1,0.0, -10000.0,-1, 0.0, -1,-1));
			int right_child_idx = tree.size();
			tree.emplace_back(Node(rhsF,curr_depth+1,0.0, -10000.0,-1, 0.0, -1,-1));
			...
			if(curr_depth < this->depth /*&& tree[node_idx].var_idx != -1*/){
				node_stack.push(left_child_idx);
				node_stack.push(right_child_idx);
				curr_depth += 1;
			}
			//std::cout<<node_idx<<" "<<node_tree_size<<" "<<tree[node_idx].idxs[0]<<" "<<tree[node_idx].idxs[1]<<std::endl;
		}
...
class XGBoostClassifier{
	...
	public:
		...
		void fit(std::vector<std::vector<double>> x, std::vector<int> y,double subsample_cols = 0.8 , int min_child_weight = 1, int depth = 8,int min_leaf = 1,double learning_rate = 0.4, int trees = 5,double lambda = 1, double gamma = 1){
			...
			std::vector<double> base_pred(x.size(), 0.0);
			
			this->base_pred = base_pred;
			
			std::vector<int> root_idxs(x.size());
		...
		std::vector<int> predict(std::vector<std::vector<double>> x){
		    ...
		    std::vector<double> sigmoid_input(x.size(), 1.0);
...
int main() {
   ...
   /*std::vector<std::vector<double>>x{{ 10.3, 20.2, 30.5, 30.5, 32.5,10.3, 20.2, 30.5, 30.5, 32.5},
    ...
    //void fit(std::vector<std::vector<double>> x, std::vector<int> y,double subsample_cols = 0.8 , int min_child_weight = 1, int depth = 8,int min_leaf = 1,double learning_rate = 0.4, int trees = 5,double lambda = 1, double gamma = 1)
    TreeGenerator mytree(x,gradient,hessian,idxs,1,1,3 ,10,1,1);
   */
    ...
    std::vector<int> y;
     ...
     // initialisation du classifieur
      ...
      top1();
      xgb.fit(x, y,0.8 ,1,8,1,0.4,100,1.5, 1);
      top2();
