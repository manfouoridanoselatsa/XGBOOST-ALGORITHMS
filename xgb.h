...
class TreeGenerator{
	...
	public:
		...
		TreeGenerator(std::vector<std::vector<double>>& x, std::vector<double>&  gradient,std::vector<double>& hessian,std::vector<int>&  idxs, double subsample_cols, int min_leaf,int min_child_weight ,int depth,double lambda,double gamma,double eps,int num_thread,int choice);
		void find_greedy_split_par(int node_idx);
		void find_greedy_split_seq(int node_idx);
		void find_greedy_split_seq_with_na(int node_idx);
		...
		void find_greedy_split_seq_approx_quantile_sketch(int node_idx);
		void find_greedy_split_greedy_approximation(int node_idx);

		std::vector<int> col_subsample(bool random_col);
		double compute_node_output( std::vector<double>& gradient, std::vector<double>& hessian);
		void find_varsplit_par();
		void find_varsplit_seq();
		void find_varsplit_par_feat();
		void find_varsplit_par_node();
		void find_varsplit_seq_with_na();
		void find_varsplit_par_feat_plus_SplitPoint();
		void find_varsplit_seq_approx_quantile_sketch();
		void find_varsplit_greedy_approximation();
...
class XGBoostClassifier{
	...
	public:
		...
		std::vector<double> fit(std::vector<std::vector<double>>& x, std::vector<int>& y,double subsample_cols, int min_child_weight, int depth,int min_leaf,double learning_rate, int trees,double lambda, double gamma,double eps,int num_threads,int choice);
